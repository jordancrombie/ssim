 ...
 *
 * This is useful for copying credentials between authenticators,
 * such as when testing with popups or iframes that have their own
 * CDP sessions.
 *
 * @param context - WebAuthn context from setupVirtualAuthenticator
 ...
 * @example
 * ```typescript
 * // Get credential from one authenticator
 * const credentials = await getStoredCredentials(mainWebauthn);
 * // Add to another authenticator
 * await addCredential(popupWebauthn, credentials[0]);
 * ```
 */
...
/**
 * Copy all credentials from one authenticator to another.
 *
 * @param source - Source WebAuthn context
 * @param target - Target WebAuthn context
 * @returns Number of credentials copied
 *
 * @example
 * ```typescript
 * // After setting up popup authenticator, copy credentials from main page
 * const copied = await copyCredentials(mainWebauthn, popupWebauthn);
 * console.log(`Copied ${copied} credentials to popup`);
 * ```
 */
export async function copyCredentials(
  source: WebAuthnContext,
  target: WebAuthnContext
): Promise<number> {
  const credentials = await getStoredCredentials(source);
  for (const cred of credentials) {
    await addCredential(target, cred);
  }
